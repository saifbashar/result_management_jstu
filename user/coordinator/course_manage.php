...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_course_code'])) {
    $course_code = $_POST['delete_course_code'];
    try {
        $sql = "DELETE FROM courses WHERE course_code = :course_code";
        $stmt = $pdo->prepare($sql);
        $stmt->bindParam(':course_code', $course_code, PDO::PARAM_STR);
        if ($stmt->execute()) {
            $success = "Course record has been successfully deleted!";
        } else {
...
try {
    $sql = "SELECT semester, course_code, course_title, credit, mark FROM courses";
    $stmt = $pdo->query($sql);
} catch (PDOException $e) {
...
<head>
    ...
    <title>Manage Courses - RAS Admin</title>
                                    ...
                                    if (isset($stmt) && $stmt->rowCount() > 0) {
                                        while ($row = $stmt->fetch()) {
                                            echo "<tr>
                                                ...
                                                <td class='actions'>
                                                    <form action='course_modify.php' method='GET' style='display:inline;'>
                                                    ...
                                                    <form action='' method='POST' style='display:inline;' onsubmit='return confirm(\"Are you sure you want to delete this course?\");'>
                                                        <input type='hidden' name='delete_course_code' value='" . htmlspecialchars($row["course_code"]) . "'>
