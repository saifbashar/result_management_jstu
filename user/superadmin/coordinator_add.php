...
$name = $email = $password = $designation = $status = $phone = '';
$error = '';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($name) || empty($email) || empty($password) || empty($designation) || empty($status) || empty($phone)) {
        $error = "Invalid input. Please ensure all fields are filled.";
    } else {
        try {
            $sql = "INSERT INTO coordinator (name, email, password, designation, status, phone, created_at) 
                    VALUES (:name, :email, :password, :designation, :status, :phone, NOW())";
...
<head>
    ...
    <title>Add Coordinator - RAS Admin</title>
