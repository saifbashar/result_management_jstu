...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_id'])) {
    ...
    try {
        ...
        if ($stmt->execute()) {
            $success = "Coordinator record has been successfully deleted!";
        } else {
...
try {
    $sql = "SELECT id, name, email, designation, status, phone, created_at FROM coordinator";
    $stmt = $pdo->query($sql);
} catch (PDOException $e) {
...
<head>
    ...
    <title>Manage Coordinators - RAS Admin</title>
        ...
        <div id="layoutSidenav_content">
            <main class="results-view-main">
                <div class="container-fluid px-4">
                    <h1 class="mt-4">Manage Coordinators</h1>
                                    ...
                                    if (isset($stmt) && $stmt->rowCount() > 0) {
                                        while ($row = $stmt->fetch()) {
                                            echo "<tr>
                                                ...
                                                <td class='actions'>
                                                    <form action='coordinator_modify.php' method='GET' style='display:inline;'>
                                                        <input type='hidden' name='id' value='" . htmlspecialchars($row["id"]) . "'>
                                                        <button type='submit' class='btn btn-primary'>Modify</button>
                                                    </form>
                                                    <form action='' method='POST' style='display:inline;' onsubmit='return confirm(\"Are you sure you want to delete this coordinator?\");'>
                                        ...
                                        echo "<tr><td colspan='8'>" . (isset($error) ? $error : "No data found") . "</td></tr>";
                                    }
                                    ?>
                                </tbody>
                            </table>
