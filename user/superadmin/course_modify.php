...
$course_code = $semester = $course_title = $credit = $mark = '';
$error = '';

if (isset($_GET['course_code'])) {
    $course_code = trim($_GET['course_code']);
    try {
        $sql = "SELECT semester, course_code, course_title, credit, mark FROM courses WHERE course_code = :course_code";
    ...
    $error = "Invalid request. No course_code provided.";
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($course_code) || empty($semester) || empty($course_title) || empty($credit) || empty($mark)) {
        $error = "Invalid input. Please ensure all fields are filled.";
    } else {
        try {
            $sql = "UPDATE courses 
                    SET semester = :semester, course_title = :course_title, credit = :credit, mark = :mark 
            ...
            if ($stmt->execute()) {
                header("Location: course_modify_success.php");
                exit;
            } else {
                $error = "Failed to update course.";
            }
        } catch (PDOException $e) {
            header("Location: course_modify_failed.php?error=" . urlencode($e->getMessage()));
            exit;
        }
    }
}
...
<head>
    ...
    <title>Modify Course - RAS Admin</title>
        ...
        <div id="layoutSidenav_content">
            <main class="results-view-main">
                <div class="container-fluid px-4">
                    <h1 class="mt-4">Modify Course</h1>
                        ...
                        <div class="card-body">
                            <form action="" method="POST" class="results-view-form">
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="text" class="form-control" id="course_code" name="course_code" value="<?php echo htmlspecialchars($course_code); ?>" readonly>
                                </div>
                                <div class="mb-3">
